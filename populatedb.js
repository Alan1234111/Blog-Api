...
async function postCreate(index, title, content, tag, comments) {
  const post = new Post({title, content, tag, comments});
  await post.save();
  posts[index] = post;
  console.log(`Added post: ${title}`);
}

async function commentCreate(index, content) {
...
async function createPosts() {
  ...
  await Promise.all([postCreate(0)]);
}
